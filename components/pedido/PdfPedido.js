...
export function PdfGracias({ pedido, items, cliente, tiendaColor }) {
  ...
  return (
    <div style={{ fontFamily:"'Helvetica Neue',Arial,sans-serif", backgroundColor:'#fff', width:'794px', minHeight:'1123px', position:'relative', overflow:'hidden', display:'table', tableLayout:'fixed' }}>
            ...
            <div style={{ backgroundColor:'#fff', borderRadius:'16px', padding:'18px 20px', marginBottom:'20px', border:'2px solid #e5e5e5' }}>
              <div style={{ display:'table', width:'100%', marginBottom:'10px' }}>
                ...
                <div style={{ display:'table-cell', verticalAlign:'middle', textAlign:'right' }}>
                  <span style={{ display:'inline-block', backgroundColor:tiendaColor, color:'#fff', fontSize:'13px', fontWeight:'800', fontFamily:'monospace', padding:'4px 12px', borderRadius:'8px' }}>
                    {pedido?.PEDIDO_ID || ''}
                  </span>
                </div>
              </div>
            ...
            {(() => {
              ...
              return (
                <div style={{ marginTop:'14px', padding:'12px 16px', borderRadius:'10px',
                  ...
                  <div style={{ display:'table-cell', width:'32px', verticalAlign:'middle', fontSize:'20px', lineHeight:1 }}>{isPagado?'✅':'🔴'}</div>
                  <div style={{ display:'table-cell', verticalAlign:'middle' }}>
                    {isPagado ? (
                      <div style={{ fontSize:'13px', fontWeight:'800', color:'#15803d' }}>PAGO COMPLETO</div>
                    ) : (
                      <>
                        <div style={{ fontSize:'13px', fontWeight:'800', color:'#b91c1c' }}>ABONO DEL {pct}% — SALDO PENDIENTE</div>
                        <div style={{ fontSize:'12px', color:'#b91c1c', marginTop:'2px', fontWeight:'700' }}>Cobrar ${saldo.toFixed(2)} al motorizado</div>
                      </>
                    )}
                  </div>
                  {!isPagado && total > 0 && (
...
// ─── HOJA 2: PÁGINA DE CONFECCIÓN (una instancia = una página A4) ─────────────
// La paginación (chunking de items) la hace el padre (impresion/page.js).
// Este componente solo recibe los items de SU página y los metadatos de paginación.
export function PdfConfeccionPagina({ pedido, items, tiendaColor, paginaActual, totalPaginas, offsetIdx, esUltimaPagina }) {
  const esMandarina = pedido?.TIENDA_ID === 'MANDARINA'
  const logo = esMandarina ? LOGO_MANDARINA : LOGO_INDSTORE
  const now = new Date()
  ...
  // Totales globales del pedido (no solo de esta página)
        ...
        <div style={{ display:'table-cell' }}>
          ...
          <div style={{ padding:'14px 48px 90px' }}>
            {(items||[]).map((item, idx) => {
              ...
              const fotoSize = `${FOTO_SIZES[fotos.length] || 150}px`
              ...
              return (
                <div key={idx} style={{ marginBottom:'16px', border:'2px solid #e5e5e5', borderRadius:'14px', overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.05)' }}>
                  ...
                  <div style={{ backgroundColor:tiendaColor+'12', borderBottom:`3px solid ${tiendaColor}`, padding:'9px 16px', display:'table', width:'100%', boxSizing:'border-box' }}>
                    ...
                    <div style={{ display:'table-cell', verticalAlign:'middle', textAlign:'right' }}>
                      {item.AREA && (
                        <span style={{ backgroundColor:tiendaColor, color:'#fff', fontSize:'10px', fontWeight:'700', padding:'3px 12px', borderRadius:'20px', textTransform:'uppercase' }}>
                          {item.AREA}
                        </span>
                      )}
                    </div>
                  ...
                  {/* Cuerpo: fotos izquierda | datos derecha */}
                  <div style={{ padding:'14px 16px', display:'table', width:'100%', boxSizing:'border-box' }}>
                            ...
                            <div key={ri} style={{ display:'table', marginBottom:ri < fotoRows.length-1 ? '6px' : '0' }}>
                              {row.map((f, fi) => (
                                <div key={fi} style={{ display:'table-cell', textAlign:'center', paddingRight:fi < row.length-1 ? '6px' : '0' }}>
                                  <img src={f.url} style={{ width:fotoSize, height:fotoSize, objectFit:'cover', borderRadius:'10px', border:'2px solid #ddd', backgroundColor:'#f5f5f5', display:'block' }} />
                    ...
                    <div style={{ display:'table-cell', verticalAlign:'top', width:'218px' }}>
                      <div style={{ display:'table', width:'100%', marginBottom:'10px' }}>
                        {fichas.filter((_,i)=>i%2===0).map(([k,v], rowIdx) => {
...
// Mantener PdfConfeccion como alias por si hay otras importaciones en el proyecto
