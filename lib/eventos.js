...
import { textoAlerta } from './alerta-texto'
...
export async function registrarEvento({ fuente, nivel = 'error', mensaje, pedidoId, detalle } = {}) {
  ...
  //
  // CON await, a propósito. Antes iba suelta y con `.catch(() => {})`: en una
  // función serverless, apenas se devuelve la respuesta la instancia se puede
  // congelar, así que el envío a Telegram quedaba volando y se perdía — el
  // mismo defecto que hacía desaparecer los eventos de `datil` de la tabla.
  // Una alerta que a veces sale no es una alerta: es una lotería.
  if (nivel === 'error') {
    try {
      const enviada = await alertarTelegram({ fuente, mensaje, pedidoId })
      if (!enviada) console.error(`ALERTA no enviada por Telegram (${fuente}): ${mensaje}`)
    } catch (e) {
      // Avisar del error no puede romper la operación que lo produjo.
      console.error('alertarTelegram falló:', e?.message || e)
    }
  }
}
...
async function alertarTelegram({ fuente, mensaje, pedidoId }) {
  ...
  // El enlace al pedido sale del dominio real donde corre el CRM, no de uno
  // escrito a mano: en Vercel el `.vercel.app` y el dominio que usa el equipo
  // (crm.apps.mandarinaec.com) NO son el mismo, y un enlace al equivocado manda
  // a la gente a pedir login de nuevo.
  const baseUrl = process.env.CRM_URL_PUBLICA || 'https://crm.apps.mandarinaec.com'
  return await enviarTelegram(chatId, textoAlerta({ fuente, mensaje, pedidoId, baseUrl }))
}
