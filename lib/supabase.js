...
export function getSupabase() {
  ...
  _client = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
    ...
    // CRÍTICO: Next.js (App Router) parchea el fetch global y cachea las
    // respuestas GET. Las lecturas de PostgREST son GET, así que sin esto
    // quedan CONGELADAS en el resultado de la primera llamada del proceso.
    //
    // Costó una noche entera el 2-ago-2026. El archivador de artes leía siempre
    // los mismos 21 anuncios pendientes sin importar lo que dijera la base:
    // reportaba "archivados: 10, errores: []" corrida tras corrida mientras el
    // conteo real era 1. Se diagnosticó mal siete veces —el filtro, los
    // comodines de PostgREST, el caché de esquema, el upsert— porque las
    // ESCRITURAS (POST/PATCH) sí funcionaban y solo mentían las LECTURAS.
    //
    // lib/inbox-supabase.js ya lo tenía; este cliente no. Cualquier cliente
    // nuevo de Supabase en este repo debe llevarlo.
