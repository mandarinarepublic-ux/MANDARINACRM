...
let _clientInbox = null;

/**
 * Cliente Supabase (service_role) apuntando al schema `inbox`
 * (conversaciones + mensajes de WhatsApp IND/MANDI). Mismo proyecto que el CRM.
 * La vista de unión crm.cliente_conversacion se lee con getSupabase() (schema crm).
 */
export function getSupabaseInbox() {
  if (_clientInbox) return _clientInbox;
  ...
  _clientInbox = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
    db: { schema: 'inbox' },
    auth: { persistSession: false, autoRefreshToken: false },
  });
