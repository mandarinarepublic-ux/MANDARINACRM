...
// ⚠️ Los registrarEvento de acá van CON await, a propósito.
//
// Estaban sueltos (fire-and-forget) y el evento nunca llegaba a la base: en una
// función serverless, apenas se devuelve la respuesta la instancia se puede
// congelar, y el insert que quedó volando se pierde. Producción lo demostró:
// el 11-ago 00:46:34 UTC el pedido IND-CAM-5589 recibió un 502 de esta ruta y
// en `crm.eventos_sistema` NO quedó ni un evento `datil`.
//
// O sea: el reporte del error se perdía exactamente cuando había un error que
// reportar. `emitirFacturaDatil` sí se espera desde la ruta, así que awaitear
// acá garantiza que el evento aterrice antes de responder. Cuesta unos ms y
// compra la única señal que tenemos de que algo se rompió.
...
export async function emitirFacturaDatil({ pedidoId, cliente, montoTotal, tipoId }) {
  // Estos dos rechazos se devolvían MUDOS: sin evento y sin alerta. Un pedido
  // que pidió factura y no la tuvo es un problema igual, aunque Dátil ni se haya
  // enterado. Se registran como cualquier otro fallo.
  const key = process.env.DATIL_API_KEY
  if (!key) {
    const m = 'DATIL_API_KEY no configurada'
    await registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: m, pedidoId })
    return { ok: false, error: m }
  }

  const total = Number(parseFloat(montoTotal || 0).toFixed(2))
  if (!(total > 0)) {
    const m = `El total debe ser mayor a 0 (llegó ${JSON.stringify(montoTotal)})`
    await registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: m, pedidoId })
    return { ok: false, error: m }
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      await registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: detalle, pedidoId })
      return { ok: false, error: detalle }
    }
  } catch (e) {
    console.error(`DATIL emitir ${pedidoId}: ${e.message}`)
    await registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: e.message, pedidoId })
  ...
  if (!datilId) {
    ...
    await registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: 'Dátil no devolvió el id de la factura', pedidoId })
  ...
  try {
    ...
    await registrarEvento({ fuente: 'datil', nivel: 'aviso', pedidoId,
      mensaje: `Factura ${doc?.numero || datilId} emitida pero no se guardó en el pedido`, detalle: { datilId } })
    return { ok: true, datilId, rideUrl, warning: 'emitida pero no guardada en el pedido' }
  }

  await registrarEvento({ fuente: 'datil', nivel: 'ok', pedidoId,
    mensaje: `Factura ${doc?.numero || datilId} emitida`, detalle: { datilId } })
