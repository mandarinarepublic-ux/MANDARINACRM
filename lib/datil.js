...
export async function emitirFacturaDatil({ pedidoId, cliente, montoTotal, tipoId }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: detalle, pedidoId })
      return { ok: false, error: detalle }
    }
  } catch (e) {
    console.error(`DATIL emitir ${pedidoId}: ${e.message}`)
    registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: e.message, pedidoId })
  ...
  if (!datilId) {
    ...
    registrarEvento({ fuente: 'datil', nivel: 'error', mensaje: 'Dátil no devolvió el id de la factura', pedidoId })
  ...
  try {
    ...
    registrarEvento({ fuente: 'datil', nivel: 'aviso', pedidoId,
  ...
  registrarEvento({ fuente: 'datil', nivel: 'ok', pedidoId,
