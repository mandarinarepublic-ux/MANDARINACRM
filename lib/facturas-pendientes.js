...
export function facturasPendientes(pedidos, { desde, ahora = new Date(), graciaMinutos = GRACIA_POR_DEFECTO } = {}) {
  ...
  const encontrados = (pedidos || []).filter((p) => {
    ...
    // Se pidió, y alguien decidió a mano que no se emite. También es una
    // decisión, no una falla.
    //
    // ☠️ Ojo con la asimetría: `factura_solicitada` es un HECHO (el cliente la
    // pidió) y `factura_descartada` es una DECISIÓN (nosotros no la emitimos).
    // Por eso son dos columnas y no una: apagar la primera para lograr el efecto
    // de la segunda borraría el hecho, y dentro de tres meses nadie sabría por
    // qué ese pedido no tiene factura.
    if (p.factura_descartada) return false
