 ...
 *
 * El tope acá es 2000 y no existe para “ahorrar”, sino para no acercarse nunca
 * al corte de PostgREST. `truncado` dice si se alcanzó, para que quien llame
 * pueda avisar en vez de mentir por omisión.
 *
 * @returns {Promise<{ids: string[], truncado: boolean}>}
 */
export async function idsClientesQueCoinciden(q, tope = 2000) {
  const term = normalizarBusqueda(q);
  ...
  return { ids, truncado: ids.length >= tope };
}
