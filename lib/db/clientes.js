...
import { limpiarEmail } from '../email-cliente.js';
...
/**
 * Solo los IDS de los clientes que coinciden con `q`. Para el Historial, que
 * busca pedidos por nombre/cédula/celular del cliente.
 *
 * ☠️ NO usa `searchClientes`: ese devuelve como mucho 50 porque alimenta un
 * desplegable, y "maria" ya trae 49 clientes reales. A la clienta 51 el
 * Historial habría empezado a perder pedidos EN SILENCIO — el mismo modo de
 * falla que veníamos matando, reintroducido por la puerta de atrás.
 *
 * ☠️ EL AVISO SALE DEL CONTEO, NO DE UN UMBRAL ADIVINADO.
 *
 * Hasta el 21-ago-2026 esto pedía 2000 "para no acercarse nunca al corte de
 * PostgREST" y avisaba con `ids.length >= tope`. Estaba al revés: 2000 está POR
 * ENCIMA del corte. PostgREST ignora un `.limit()` mayor al suyo y devuelve 1000
 * sin decir nada — y entonces `1000 >= 2000` es `false`, o sea que **el aviso que
 * pusimos para no mentir por omisión iba a mentir justo el día que se truncara**.
 *
 * El `count` de PostgREST NO se trunca (misma técnica que `total_prendas` en
 * lib/db/impresion.js, la que hizo que el papel delatara al IND-XAV-5641). Si
 * llegaron menos de los que hay, se sabe con certeza y no por corazonada.
 *
 * ⚠️ Bajar el tope por debajo de 1000 NO es la solución: recortaría búsquedas que
 * hoy funcionan (con 913 clientes, la letra "a" ya coincide con 872).
 *
 * Tampoco se pagina hasta traerlos todos: con más de 1000 coincidencias la
 * búsqueda no le sirve a nadie, y estos ids viajan DENTRO DE LA URL del `.or()`
 * de historial. Es mejor avisar y pedir un término más específico.
 *
 * @returns {Promise<{ids: string[], truncado: boolean}>}
 */
export async function idsClientesQueCoinciden(q, tope = 1000) {
  const term = normalizarBusqueda(q);
  if (!term) return { ids: [], truncado: false };
  ...
  const ids = (data || []).map((c) => c.cliente_id).filter(Boolean);
  // Solo evidencia POSITIVA marca la búsqueda truncada: si el conteo no llega,
  // se asume completa. El peor caso es quedarse sin aviso, nunca inventarlo.
  const hayConteo = typeof count === 'number' && Number.isFinite(count);
  return { ids, truncado: hayConteo && ids.length < count };
}
...
export async function createCliente({ nombre, cedula, celular, email, ciudad, direccion }) {
  // ☠️ Se limpia ACÁ y no en la pantalla: hay tres caminos de escritura (nuevo
  // pedido, editar cliente, y el upsert por cédula del POST de pedidos), y basta
  // con olvidarse en uno para que sigan entrando correos con espacios en medio.
...
export async function updateCliente(id, { nombre, cedula, celular, email, ciudad, direccion } = {}) {
  email = limpiarEmail(email);   // `undefined` sigue siendo `undefined`: es parcial
  await write({
    sheets: async () => {
