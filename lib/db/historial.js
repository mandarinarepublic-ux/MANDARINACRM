...
import { searchClientes } from './clientes'
...
function aplicarAlcance(consulta, usuario) {
  ...
  if (rol === 'VENDEDOR') {
    ...
    // aceptan los dos, que es lo que hacía el filtro viejo.
    const suyos = [usuario?.NOMBRE ?? usuario?.nombre, usuario?.USUARIO_ID ?? usuario?.id]
...
export async function listHistorial({
  ...
  if (termino) {
    const clientes = await searchClientes(termino, 50)
    const ids = clientes.map((c) => c.CLIENTE_ID).filter(Boolean)
  ...
  return { pedidos, total, hayMas, pagina: pag }
}
