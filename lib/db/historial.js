...
import { esAreaFiltrable } from '../areas-filtrables.js'
import { veTodasLasVentas } from '../tiendasUsuario.js'
import { conteoEsFiable, resumenPagina, esFueraDeRango } from '../historial-paginacion.js'
...
// El select cuando se filtra por área.
//
// ☠️ El `!inner` va en un SEGUNDO embed de la misma tabla, con otro alias, que
// no se lee nunca. Si el filtro fuera sobre `prendas` — el embed que se PINTA —
// PostgREST recortaría también las prendas devueltas: un pedido de 3 prendas con
// una sola de bordado se vería como un pedido de UNA prenda. Esa es exactamente
// la trampa de "la lista recortada con cara de completa" que ya costó cara acá.
//
// Con dos embeds, el `!inner` decide QUÉ PEDIDOS salen y `prendas` sigue
// trayéndolas todas.
...
function aplicarAlcance(consulta, usuario) {
  ...
  // El permiso `VER_TODAS_LAS_VENTAS` levanta el filtro por vendedor — y SOLO
  // ese. El de tienda, más abajo, se sigue aplicando igual: si no, quien lo
  // tenga vería también YAW, que no le toca.
...
export async function listHistorial({
  usuario, pagina = 0, estado, tienda, pago, desde, hasta, busqueda, area,
} = {}) {
  const sb = getSupabase()
  const pag = Math.max(0, Number(pagina) || 0)

  // Solo se acepta un área de la lista: lo que llegue por la url no puede
  // convertirse en un patrón de búsqueda cualquiera contra la base.
  const areaPedida = area && area !== 'TODAS' && esAreaFiltrable(area)
    ? String(area).toUpperCase()
    : null

  // El join extra SOLO cuando hace falta: sin filtro, la consulta de siempre.
  let consulta = sb.from('pedidos')
    .select(areaPedida ? SELECT_CON_AREA : SELECT, { count: 'exact' })
  consulta = aplicarAlcance(consulta, usuario)

  // ☠️ `ilike '%AREA%'`, NUNCA `eq`. `area` guarda combinaciones en un solo
  // texto (`ESTAMPADO + BORDADO`, 82 pedidos): con `=` se perderían 77 pedidos
  // de bordado y 75 de estampado, sin que la pantalla lo note.
  if (areaPedida) consulta = consulta.ilike('filtro_area.area', `%${areaPedida}%`)
  ...
  // ☠️ Un tramo que empieza pasada la última fila NO es una avería: es una
  // página que ya no existe. PostgREST contesta 416 y eso llegaba al usuario
  // como "El Historial fallo al cargar" (4-sep-2026, 19:45). Cualquier OTRO
  // error sí se propaga: tragárselos haría que "no hay pedidos" y "no pude
  // leer" se vieran idénticos.
  if (error && esFueraDeRango(error)) {
    return { pedidos: [], total: null, hayMas: false, pagina: pag, busquedaTruncada }
  }
  if (error) throw error

  const pedidos = (data || []).map(aPedido)

  // ☠️ Con filtro de área el `count` NO es de fiar: `filtro_area:...!inner(area)`
  // es un join a una relación de muchos, así que cuenta FILAS DEL JOIN. Medido
  // el 4-sep-2026: ESTAMPADO 441 pedidos → 733 filas (1,66×). Publicarlo hacía
  // que la cabecera mintiera y que "Ver más" sobreviviera a la última página
  // hasta pedir un tramo inexistente. Ver lib/historial-paginacion.js.
  const { total, hayMas } = resumenPagina({
    conteo: count,
    recibidas: pedidos.length,
    primera,
    tamano: TAMANO_PAGINA,
    fiable: conteoEsFiable({ areaPedida }),
  })
