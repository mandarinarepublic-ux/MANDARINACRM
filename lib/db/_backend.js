...
/**
 * ¿Se sigue llenando Google Sheets como copia?
 *
 * ☠️ APAGADO desde el 19-ago-2026, por decisión de Rodrigo. Razones:
 *
 *  · La copia iba *best-effort*: sin `await` y con un `.catch` que solo hacía
 *    `console.warn`. Nadie miraba esos warnings, así que **no se sabe si la hoja
 *    está completa**: pudo llevar semanas perdiendo filas en silencio. Una copia
 *    que nadie verifica no es un respaldo, es una ilusión que tranquiliza.
 *  · Cuesta latencia y cuota de la API de Google en cada venta, cambio de
 *    estado, pago y guía.
 *  · Supabase es la fuente de verdad desde el cutover, y tiene sus propios
 *    respaldos.
 *
 * Se vuelve a encender poniendo DUAL_WRITE_SHEETS=1 en Vercel. No se borró el
 * código de las escrituras a Sheets: siguen ahí, solo no se llaman.
 *
 * ⚠️ Si algún día se pone DATA_BACKEND=sheets, la hoja será la fuente de verdad
 * pero estará CONGELADA desde hoy. Por eso `write()` avisa a gritos en ese caso.
 */
export const DUAL_WRITE_SHEETS =
  process.env.DUAL_WRITE_SHEETS === '1' || process.env.DUAL_WRITE_SHEETS === 'true';
...
export async function write({ sheets, supabase }) {
  ...
  // ⚠️ Si Sheets fuera la PRIMARIA, la hoja lleva congelada desde el 19-ago-2026
  // (ver DUAL_WRITE_SHEETS). Escribir ahí como fuente de verdad significaría
  // partir los datos en dos, así que se avisa a gritos en cada operación.
  if (primaryKey === 'sheets' && !DUAL_WRITE_SHEETS) {
    console.error(
      '[dual-write] DATA_BACKEND=sheets pero la copia a Sheets esta APAGADA desde el 19-ago-2026: ' +
      'la hoja no tiene lo de Supabase. Pon DATA_BACKEND=supabase o DUAL_WRITE_SHEETS=1.'
    );
  }

  // La secundaria de Supabase sólo corre si hay credenciales.
  // La de Sheets, solo si el dual-write sigue encendido.
  const secondaryEnabled =
    typeof secondary === 'function' &&
    (secondaryKey === 'sheets' ? DUAL_WRITE_SHEETS : supabaseConfigured());
