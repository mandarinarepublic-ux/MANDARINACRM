...
export async function setFactura(pedidoId, { datilId, rideUrl } = {}) {
  ...
  await write({
    ...
    supabase: async () => {
      // `.select()` para saber CUÁNTAS filas se tocaron. Sin esto, un UPDATE que
      // no encuentra el pedido devuelve `error: null` y esto reportaba éxito
      // habiendo guardado nada. Verificado en producción el 11-ago-2026: un POST
      // a /api/factura-callback con un pedido_id inexistente contestó `ok:true`.
      // Con una factura ya emitida en el SRI de por medio, "guardé" y "no guardé
      // nada" no pueden verse igual.
      ...
      if (!data || data.length === 0) throw new Error(`Pedido ${pedidoId} no encontrado`);
    },
  });
}
