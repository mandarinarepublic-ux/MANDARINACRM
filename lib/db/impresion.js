...
const COLS_PEDIDO = [
  ...
  // ☠️ El PAGO. Sin estas dos, la hoja del cliente sale SIEMPRE en rojo diciendo
  // "ABONO DEL 0% — SALDO PENDIENTE" y cobrando de nuevo el total entero, porque
  // components/pedido/PdfPedido.js lee `MONTO_ABONADO` y `ESTADO_PAGO` y aqui
  // llegaban `undefined`. Medido el 26-ago-2026: de 81 pedidos EN_FABRICA, 67
  // estaban PAGADOS. La pantalla del pedido salia bien — esa lee
  // /api/pedidos/{id}, que si los trae — asi que el fallo era SOLO del lote.
  'estado_pago', 'monto_abonado',
].join(',')
...
function aPedido(p) {
  ...
  return {
    ...
    // Mismos nombres y mismo formato que lib/db/pedidos.js: la hoja es la misma
    // en el lote y en la pantalla del pedido, asi que tiene que leer lo mismo.
