...
/**
 * SUBESTADO_CORTE de VARIAS prendas de un tirón (la bandeja de Corte actuando
 * sobre el pedido entero).
 *
 * ☠️ UNA sola escritura con `.in()`, no N llamadas en fila: un pedido de seis
 * prendas serían seis viajes a la base desde una función serverless, y si el
 * cuarto falla el pedido queda a medio marcar sin que nadie se entere.
 *
 * ⚠️ La lista llega de `lib/cortePedido.js`, que ya sacó las prendas que YA
 * estaban en ese estado: acá no se re-filtra. Con la lista vacía no se escribe
 * nada (un `.in()` sin ids en PostgREST no es "ninguna fila", es un filtro que
 * no filtra).
 *
 * @param {string[]} itemIds
 * @param {string} valor  PENDIENTE | SOLICITADO | CORTADO
 * @returns {Promise<number>} cuántas prendas se mandaron a escribir
 */
export async function updateSubestadoCorteLote(itemIds, valor) {
  const ids = (Array.isArray(itemIds) ? itemIds : []).filter(Boolean)
  if (ids.length === 0) return 0

  await write({
    sheets: async () => {
      // La hoja no tiene escritura por lote: se recorre. Va best-effort y hoy
      // apagada (DUAL_WRITE_SHEETS), pero se mantiene la paridad por si vuelve.
