...
/** Lectura SOLO Supabase (para lecturas-sombra), con los mismos filtros. */
export async function listProductosShopifySupabase({ tienda, q } = {}) {
  let query = getSupabase().from('productos_shopify').select('*').eq('activo', true);
