...
export async function addMensaje(conversacionId, opts = {}) {
  const {
    mensajeId, waMessageId, cuenta, telefono, nombre, direccion, tipo, texto,
  ...
  // Idempotencia por id externo (wamid de Meta / ID del sheet).
  if (waMessageId) {
    const { data: existente } = await db
      .from('mensajes').select('*').eq('wa_message_id', waMessageId).maybeSingle();
 ...
 */
export async function recibirMensaje({ cuenta, telefono, nombreContacto, waId, canal, waMessageId, tipo, texto, mediaUrl, mediaId, contextoId, fecha } = {}) {
  ...
  const mensaje = await addMensaje(conversacion.conversacion_id, {
    waMessageId, cuenta, telefono, nombre: nombreContacto,
 ...
 */
export async function enviarMensaje({ conversacionId, cuenta, telefono, nombreContacto, waMessageId, tipo, texto, mediaUrl, mediaId, respuestaIa, contextoId, fecha } = {}) {
