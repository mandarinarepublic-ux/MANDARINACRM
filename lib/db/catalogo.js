...
export async function listCatalogoGestion() {
  ...
  // ☠️ El conteo lo hace la BASE (vista `crm.uso_tipos_prenda`), no este bucle.
  //
  // Antes se leían TODAS las filas de `detalle_pedido` para contarlas acá: 1314
  // cuando PostgREST devuelve 1000 como mucho, así que **314 se perdían** y el
  // número salía corto.
  //
  // No era cosmético: este conteo existe para que el admin sepa qué puede borrar
  // sin pensarlo. Un tipo con "0 usos" que en realidad se usó se borra, y el
  // historial de esos pedidos queda apuntando a algo que ya no existe.
  //
  // La vista devuelve un nombre por fila (208 hoy), muy lejos del tope.
  const { data: usados, error: e2 } = await sb
    .from('uso_tipos_prenda')
    .select('nombre, usos');
  if (e2) throw e2;

  const conteo = new Map();
  for (const d of usados || []) {
    const k = String(d.nombre || '').trim().toUpperCase();
    if (k) conteo.set(k, Number(d.usos) || 0);
  }
