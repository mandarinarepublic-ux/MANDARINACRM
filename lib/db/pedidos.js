...
const CAMPOS = [
  ...
  // direccion: en Sheets se duplica en dos columnas; en Postgres una sola.
  ['direccionPedido',          ['DIRECCION_TEXTO', 'DIRECCION_PEDIDO'],  'direccion_pedido',           'text'],
  ...
  ['diasCalculado',            ['DIAS_CALCULADO'],                       'dias_calculado',             'num'],
  ['diasPrometido',            ['DIAS_PROMETIDO'],                       'dias_prometido',             'num'],
...
function pedidoSupToSheet(p) {
  return {
    ...
    DIAS_CALCULADO: p.dias_calculado != null ? String(p.dias_calculado) : '',
    DIAS_PROMETIDO: p.dias_prometido != null ? String(p.dias_prometido) : '',
  ...
  await write({
    ...
    sheets: async () => {
      await appendRow('PEDIDOS', [
        ...
        notasVendedor || '', guiaNumero || '', guiaTransportista || '',
        direccionPedido || '',
        latitud != null && latitud !== '' ? String(latitud) : '',
        longitud != null && longitud !== '' ? String(longitud) : '',
      ]);
    },
    supabase: async () => {
...
async function writePedidoSheets(id, sheetUpdates) {
  ...
  // Merge sobre la fila existente + siempre toca FECHA_ACTUALIZACION.
  const updated = { ...pedidos[idx], ...sheetUpdates, FECHA_ACTUALIZACION: fechaAhora() };
