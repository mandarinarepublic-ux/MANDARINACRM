...
/** TODOS los PEDIDO_ID existentes, respetando DATA_BACKEND (para el anti-colisión del POST). */
export async function listPedidoIds() {
  return read({
    sheets: async () => (await readSheet('PEDIDOS')).map((p) => p.PEDIDO_ID),
    supabase: async () => {
      const { data, error } = await getSupabase().from('pedidos').select('pedido_id');
      if (error) throw error;
      return (data || []).map((p) => p.pedido_id);
    },
  });
}
