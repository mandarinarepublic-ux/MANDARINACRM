...
const CAMPOS = [
  ...
  ['direccionPedido',          ['DIRECCION_PEDIDO'],                     'direccion_pedido',           'text'],
  ...
  await write({
    ...
    sheets: async () => {
      await appendRow('PEDIDOS', [
        ...
        direccionPedido || '',
        '', '',                 // FACTURA_ID, FACTURA_PDF_URL (se llenan al emitir factura)
      ]);
      // NOTA: guiaNumero/guiaTransportista/latitud/longitud NO tienen columna en la
      // hoja PEDIDOS — viven solo en Supabase (abajo) y en la hoja GUIAS_DESPACHO.
    },
    supabase: async () => {
