...
import { read, write, toBool, boolStr, toNum, toTimestamp, BACKEND } from './_backend';
...
const CAMPOS = [
  ...
  ['facturaDescartada',        ['FACTURA_DESCARTADA'],                   'factura_descartada',         'bool'],
...
function pedidoSupToSheet(p) {
  return {
    ...
    FACTURA_DESCARTADA: boolStr(p.factura_descartada),
    FACTURA_DESCARTADA_MOTIVO: p.factura_descartada_motivo ?? '',
    FACTURA_DESCARTADA_NOTA: p.factura_descartada_nota ?? '',
    FACTURA_DESCARTADA_POR: p.factura_descartada_por ?? '',
...
/**
 * Tope de la lista sin filtrar.
 *
 * ☠️ Existe para que esta consulta deje de MENTIR. Sin él, PostgREST devolvía
 * 1000 filas y `error: null` — nadie se enteraba de que faltaban pedidos. Con un
 * `.limit()` explícito la lista sigue estando recortada, pero por una decisión
 * escrita y no por un límite invisible.
 *
 * Está por debajo de 1000 a propósito: así el recorte es SIEMPRE el nuestro.
 */
const LIMITE_LISTA = 500;

/**
 * Cuántos pedidos se preguntan de una vez al traer sus tablas hijas.
 *
 * ☠️ POR QUÉ EXISTE: este lector pedía las prendas de los 500 pedidos en UNA
 * sola consulta. Medido el 21-ago-2026: 969 prendas. A 31 filas del corte de
 * 1000 de PostgREST — unos 16 pedidos, o sea día y medio al ritmo actual.
 *
 * Al cruzarlo, PostgREST habría devuelto 1000 filas con `error: null` y los
 * pedidos del final habrían salido SIN PRENDAS. Es exactamente lo que le pasó al
 * IND-XAV-5641 el 17-ago: se fabricó de menos y nadie se enteró.
 *
 * 120 pedidos por tanda ≈ 230 prendas: aunque el promedio se duplique, no se
 * acerca a 1000. Y si mañana un pedido trae 50 prendas, sigue sobrando espacio.
 */
const PEDIDOS_POR_TANDA = 120;

/**
 * Trae las filas hijas de una lista de pedidos SIN poder truncarse.
 *
 * ⚠️ El `.in()` viaja dentro de la URL, así que las tandas también la mantienen
 * corta. Nunca cambies esto por una sola consulta con todos los ids.
 */
async function porTandas(sb, tabla, ids, columna, ajustar) {
  const limpios = [...new Set((ids || []).filter(Boolean))];
  if (limpios.length === 0) return [];

  const filas = [];
  for (let i = 0; i < limpios.length; i += PEDIDOS_POR_TANDA) {
    const tanda = limpios.slice(i, i + PEDIDOS_POR_TANDA);
    let q = sb.from(tabla).select('*').in(columna, tanda);
...
async function joinSupabase(opts = {}, soloId = null) {
  ...
  // El `.limit()` va desde el principio, en la MISMA sentencia: si se aplicara
  // después, bastaría con olvidarlo en una rama para volver a traerlo todo.
  // Con `soloId` es una fila igual, así que no estorba.
  let pq = sb.from('pedidos').select('*').limit(LIMITE_LISTA);
  if (soloId) pq = pq.eq('pedido_id', soloId);
  // Sin id concreto, los más recientes. Ninguna pantalla usa esta lista: todas
  // tienen su cola propia. Si necesitas una lista nueva, haz otra cola — no
  // subas este número.
  else pq = pq.order('unique_id', { ascending: false });
  ...
  const [det, pag, cli, gui] = await Promise.all([
    porTandas(sb, 'detalle_pedido', pedidoIds, 'pedido_id', (q) => q.eq('eliminado', false)),
    porTandas(sb, 'pagos', pedidoIds, 'pedido_id'),
    porTandas(sb, 'clientes', clienteIds, 'cliente_id'),
    porTandas(sb, 'guias_despacho', pedidoIds, 'pedido_id'),
  ]);

  const detalles = det.map(detalleSupToSheet);
  const pagos = pag.map(pagoSupToSheet);
  const clientes = cli.map(clienteSupToSheet);
  const guias = gui.map(guiaSupToSheet);
...
// Acá vivía `listPedidoIds()`: "TODOS los PEDIDO_ID existentes", que alimentaba
// la red anti-colisión del POST. Traía la tabla entera para calcular un número,
// y a partir de 1000 filas PostgREST habría devuelto un máximo falso.
//
// Se fue con la red: ahora el número sale de `siguienteNumeroPedido()`, que lee
// UNA fila (`order(unique_id desc).limit(1)`), y la garantía contra duplicados la
// da el índice único `pedidos_unique_id_key`.
//
// Se borra en vez de dejarla: una función que trae la tabla entera es una trampa
// esperando a que alguien la llame.

/**
 * Versión LIVIANA para el hook de notificación de pedidos nuevos
 * (useNuevosPedidos): devuelve SOLO los pedidos EN_FABRICA con su PEDIDO_ID,
 * ESTADO_PEDIDO y las AREAS de sus items — sin el join pesado de pagos/cliente/
 * guía ni el resto de campos. Mismo shape que el hook ya consume, así la lógica
 * de detección no cambia; solo baja el peso por llamada (2 lecturas en vez de 5
 * y payload mínimo).
 */
export async function listPedidosRecientes() {
  return read({
    sheets: async () => {
      const pedidos = (await readSheet('PEDIDOS')).filter((p) => p.ESTADO_PEDIDO === 'EN_FABRICA');
      if (pedidos.length === 0) return [];
      const ids = new Set(pedidos.map((p) => p.PEDIDO_ID));
      const detalles = (await readSheet('DETALLE_PEDIDO')).filter(
        (d) => ids.has(d.PEDIDO_ID) && d.SUBESTADO !== 'ELIMINADO'
      );
      return pedidos.map((p) => ({
        PEDIDO_ID: p.PEDIDO_ID,
        ESTADO_PEDIDO: p.ESTADO_PEDIDO,
        items: detalles.filter((d) => d.PEDIDO_ID === p.PEDIDO_ID).map((d) => ({ AREA: d.AREA ?? '' })),
      }));
    },
    supabase: async () => {
      const sb = getSupabase();
      const { data: pedRows, error: e1 } = await sb
        .from('pedidos')
        .select('pedido_id, unique_id, estado_pedido')
        .eq('estado_pedido', 'EN_FABRICA');
      if (e1) throw e1;
      const pedidos = pedRows || [];
      if (pedidos.length === 0) return [];
      const ids = pedidos.map((p) => p.pedido_id);
      const { data: detRows, error: e2 } = await sb
        .from('detalle_pedido')
        .select('pedido_id, area')
        .eq('eliminado', false)
        .in('pedido_id', ids);
      if (e2) throw e2;
      const det = detRows || [];
      return pedidos.map((p) => ({
        PEDIDO_ID: p.pedido_id,
        // El NUMERO. Sin el, el hook ordenaba `MAN-AND-5677` como texto: primero
        // por tienda y vendedor, y solo al final por numero.
        UNIQUE_ID: p.unique_id,
        ESTADO_PEDIDO: p.estado_pedido,
        items: det.filter((d) => d.pedido_id === p.pedido_id).map((d) => ({ AREA: d.area ?? '' })),
      }));
    },
  });
}
...
export async function createPedido(data) {
  const {
    pedidoId, uniqueId, tiendaId, vendedorId, clienteId,
    ...
    // Atribución, congelada al grabar. Ver lib/origenPedido.js.
    origen = null, origenAdId = null, origenCtwaClid = null,
    origenTelefono = null, origenAt = null,
  } = data;
  ...
  await write({
    ...
    supabase: async () => {
      const { error } = await getSupabase().from('pedidos').insert({
        pedido_id: pedidoId,
        // El número como NÚMERO. Tiene índice único: si dos ventas simultáneas
        // llegan con el mismo, la base rechaza la segunda y quien llama reintenta.
        // Esa es la garantía real de que no hay duplicados.
        unique_id: uniqueId,
        tienda_id: tiendaId,
        vendedor_id: vendedorId || null,   // nombre o uuid, tal cual venga (sin forzar FK)
        cliente_id: clienteId || null,
        // toTimestamp añade el offset -05:00 al string de Ecuador ("13Jul2026 18:23:10");
        // sin él Postgres (sesión UTC) lo marcaba como +00 y guardaba la hora 5h antes.
        fecha_pedido: toTimestamp(fechaPedido) || nowIso,
        fecha_actualizacion: toTimestamp(fechaActualizacion) || nowIso,
        ...
        // De dónde vino la venta. Se escribe UNA vez: recalcularla después haría
        // que un reporte de julio cambiara solo en octubre.
        origen,
        origen_ad_id: origenAdId,
        origen_ctwa_clid: origenCtwaClid,
        origen_telefono: origenTelefono,
        origen_at: origenAt,
      });
      if (error) throw error;
    },
 ...
 *
 * Cada backend guarda su propio formato (la hoja usa "21Jul2026 14:33:00" y
 * Postgres un timestamp ISO); ambos los entiende lib/parseFecha, así que la UI
 * los muestra igual. Devuelve el valor del backend PRIMARIO para que quien llame
 * registre y muestre exactamente lo que quedó guardado, en vez de inventarlo.
 */
export async function markImpreso(id, usuarioId) {
  const fechaSheets = fechaAhora();
  const fechaIso = new Date().toISOString();

  await write({
    sheets: () => writePedidoSheets(id, {
      FECHA_IMPRESION_PRODUCCION: fechaSheets,
      IMPRESO_POR: usuarioId || '',
    }),
    supabase: () => writePedidoSupabase(id, {
      fecha_impresion_produccion: fechaIso,
      impreso_por: usuarioId || null,
    }),
  });

  // BACKEND es la MISMA constante que decide de dónde se lee, así que la fecha
  // devuelta es siempre la que la UI volverá a leer.
  return { fecha: BACKEND === 'supabase' ? fechaIso : fechaSheets, usuario: usuarioId || '' };
}

// ─── Corte por pedido ─────────────────────────────────────────────────────────

/**
 * Guarda el resultado de una acción de corte sobre el PEDIDO entero: la traba
 * («falta algo», con su texto, fecha y quién) o el cierre («cortado todo»).
 *
 * Las columnas las arma `lib/cortePedido.js`; acá solo se escriben tal cual.
 *
 * ⚠️ NO pasa por `updatePedido`: esa función filtra por la lista blanca `CAMPOS`
 * y lo que no esté ahí **se descarta en silencio** — la pantalla diría que
 * guardó y al recargar no estaría (le pasó a `opciones` en cotizaciones). Acá
 * las columnas van directas.
 *
 * ⚠️ SOLO SUPABASE, sin espejo a Sheets: estas columnas nacen el 22-sep-2026 y
 * la hoja quedó congelada el 19-ago. No existe columna que espejar; inventarle
 * un header nuevo a una hoja que nadie lee sería costo sin lector.
 *
 * ☠️ Esta escritura SÍ puede lanzar, a diferencia de la bitácora: si falla, el
 * cortador tiene que enterarse. Una traba que se cree guardada y no lo está es
 * peor que no poder trabar — el pedido se le desaparece de la bandeja igual.
 *
 * @param {string} pedidoId
 * @param {object} columnas  corte_pendiente_* / corte_terminado_*
 */
export async function updateCorteDePedido(pedidoId, columnas = {}) {
  if (!pedidoId) throw new Error('Falta el pedido')
  const { error } = await getSupabase()
    .from('pedidos')
    .update({ ...columnas, fecha_actualizacion: new Date().toISOString() })
    .eq('pedido_id', pedidoId)
  if (error) throw error
}

/**
 * ¿Este pedido tiene traba de corte puesta? Lectura de UNA columna.
 *
 * Existe aparte de `getPedidoById` porque corre en el camino caliente: cada vez
 * que el taller mueve una prenda. Traer el pedido entero con sus cinco tablas
 * para leer un texto sería pagar el precio del join en cada clic del área.
 *
 * ☠️ Falla ABIERTO (`false` si la lectura revienta): un fallo leyendo la traba no
 * puede impedir que el área registre su trabajo. El peor caso es que el
 * auto-marcado corra una vez de más, que es exactamente lo que pasaba antes.
 *
 * @returns {Promise<boolean>}
 */
export async function tieneTrabaDeCorte(pedidoId) {
  if (!pedidoId) return false
  try {
    const { data, error } = await getSupabase()
      .from('pedidos')
      .select('corte_pendiente_nota')
      .eq('pedido_id', pedidoId)
      .maybeSingle()
    if (error) throw error
    return String(data?.corte_pendiente_nota ?? '').trim().length > 0
  } catch (e) {
    console.error('tieneTrabaDeCorte:', e?.message || e)
    return false
  }
}
...
/** El número base si la tabla estuviera vacía. Histórico del CRM. */
const NUMERO_BASE = 2400;

/**
 * El siguiente número de pedido.
 *
 * ☠️ ANTES traía TODOS los `pedido_id` y sacaba el máximo en JavaScript. Dos
 * problemas, los dos medidos el 19-ago-2026:
 *
 *   · PostgREST devuelve 1000 filas como mucho, y en orden FÍSICO. El pedido de
 *     número más alto vive al final de la tabla (posición 670 de 676), que es
 *     justo lo que el corte deja fuera: pasadas las 1000 filas el código habría
 *     visto un máximo falso y generado un número ya usado.
 *   · La otra copia de esta función (`lib/pedidos.js`) leía la hoja de Google
 *     Sheets, así que **crear una venta dependía de Sheets** aunque la fuente de
 *     verdad sea Supabase: si la hoja fallaba, no se podía vender.
 *
 * Ahora se pide UNA fila: `order(unique_id desc).limit(1)` entra por el final
 * del índice. Medido: 0,147 ms con `Heap Fetches: 0`, contra 5,5 ms leyendo las
 * 676 filas. Y es CONSTANTE — a 10 mil pedidos sigue leyendo una sola fila.
 *
 * NO reserva nada: el número se pide al grabar. Si la venta no se completa,
 * ninguna fila se escribió y el número sigue libre para el siguiente. Por eso
 * no se usa una secuencia: `nextval()` no devuelve el número en un rollback y
 * dejaría huecos.
 */
export async function siguienteNumeroPedido() {
  const { data, error } = await getSupabase()
    ...
    .limit(1);
  if (error) throw error;
  const maximo = data?.[0]?.unique_id;
  return Number.isFinite(Number(maximo)) ? Number(maximo) + 1 : NUMERO_BASE;
}

/**
 * Genera el PEDIDO_ID `PREFIJO-COD-NUM` y el número suelto.
 *
 * El `pedido_id` se sigue armando acá y no en la base: `pedidos.vendedor_id`
 * guarda a veces el nombre y a veces un uuid, y el código del vendedor vive en
 * otra tabla (con casos como JACKELINE → `JACK`, que se recorta a `JAC`). Un
 * trigger que tenga que resolver eso por nombre es frágil justo en el camino
 * crítico de una venta.
 *
 * @returns {Promise<{pedidoId: string, uniqueId: number}>}
 */
export async function generatePedidoId(tiendaId, vendedorCodigo) {
  ...
  const uniqueId = await siguienteNumeroPedido();
  return { pedidoId: `${prefix}-${code}-${uniqueId}`, uniqueId };
}
