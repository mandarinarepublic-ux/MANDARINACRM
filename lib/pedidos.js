...
import { logCambio as logCambioDual } from '@/lib/db/logs'
...
// Delega en el repositorio dual-write (lib/db/logs): escribe la bitácora en el
// backend de verdad (Sheets en Fase 3) y en espejo en Supabase. Misma firma que
// antes, así que los ~12 llamadores no cambian. NO-THROW igual que la versión previa.
export async function logCambio(pedidoId, campo, valorAntes, valorDespues, usuarioId) {
  await logCambioDual(pedidoId, campo, valorAntes, valorDespues, usuarioId)
}
