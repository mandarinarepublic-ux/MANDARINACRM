...
/**
 * Quién llama, verificado CONTRA LA BASE, para rutas que no exigen ser ADMIN.
 *
 * Es el hermano menor de `requireAdmin`: misma identidad (la cookie firmada),
 * mismas comprobaciones de que el usuario exista y esté activo, pero devuelve
 * el usuario en vez de exigir un rol. Lo usan las rutas donde el permiso NO es
 * "sos admin o no entras" sino "cada quien ve lo suyo, el admin ve todo".
 *
 * ⚠️ El rol sale de la BASE, no de la cookie, aunque la cookie esté firmada y
 * traiga uno. La firma garantiza que nadie la alteró, no que siga siendo
 * cierta: la cookie dura 30 días, así que a quien degradan hoy le queda un mes
 * de token diciendo que es ADMIN. Releer es una consulta y cierra esa ventana.
 *
 * ⚠️ Devuelve `ok:false` para los TOKENS DE MÁQUINA (`CRM_API_TOKEN`): pasan el
 * middleware pero no son una persona, así que no tienen "lo suyo". Quien llame
 * decide qué hacer con ellos — ver el ⚠️ de app/api/cotizaciones/route.js.
 *
 * @returns {Promise<{ok:true, usuario:object} | {ok:false, status:number, error:string}>}
 */
export async function usuarioDeSesion() {
  ...
  try {
    ...
    console.error('usuarioDeSesion: fallo verificando la sesión:', e?.message || e)
    return { ok: false, status: 500, error: 'No se pudo verificar la sesión' }
  }
