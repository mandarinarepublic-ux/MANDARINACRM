...
/** ¿Este usuario vende, aunque su rol no sea de venta? (permiso por persona) */
export function puedeVender(user) {
  const accesos = user?.accesos ?? user?.ACCESOS
  return (Array.isArray(accesos) ? accesos : []).includes('VENTAS')
}

/**
 * ¿Este usuario ve las ventas de TODOS, no solo las suyas? (permiso por persona)
 *
 * El rol VENDEDOR filtra el Historial por `vendedor_id`: cada quien ve lo suyo.
 * Este permiso levanta ESE filtro y nada más.
 *
 * ☠️ NO levanta el filtro por TIENDA. Son dos cosas distintas y hay que
 * mantenerlas separadas: quien lo tiene sigue viendo solo las tiendas que tenga
 * asignadas. Mezclarlas le abriría YAW (129 pedidos) a alguien que solo debía
 * ver MANDARINA e INDSTORE.
 *
 * Y NO toca "Mis Pedidos": esa pantalla significa los MÍOS. Si mostrara los de
 * todos, mentiría en su propio nombre.
 */
export function veTodasLasVentas(user) {
  const accesos = user?.accesos ?? user?.ACCESOS
  return (Array.isArray(accesos) ? accesos : []).includes('VER_TODAS_LAS_VENTAS')
}
 ...
 *
 * ⚠️ Quien tiene el permiso VENTAS cuenta como rol de venta para esto, aunque
 * sea de DISEÑO. Si no, un diseñador con permiso de vender podría registrar una
 * venta de CUALQUIER tienda por API — incluida YAW, que no le toca. La pantalla
 * solo ofrece MANDARINA e INDSTORE, pero esconder no es restringir.
 */
export function filtraPorTienda(user) {
  if (!user || user.rol === 'ADMIN') return false
  if (!ROLES_FILTRADOS.includes(user.rol) && !puedeVender(user)) return false
