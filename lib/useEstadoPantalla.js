...
import { almacenLocal, guardar, leer, hayFiltro, filtrosActivos } from './estado-pantalla'
...
export function useEstadoPantalla(pantalla, porDefecto, opciones = {}) {
  ...
  /** Quita UN filtro (la × de su chip) devolviéndolo a su valor por defecto. */
  const quitarFiltro = useCallback((campo) => {
    setValores((v) => ({
      ...v,
      [campo]: porDefectoRef.current[campo],
      ...alFiltrarRef.current,
    }))
  }, [])
  ...
  /** Los filtros puestos, para pintarlos como chips. `etiquetas` es opcional. */
  const chipsDe = useCallback((etiquetas) =>
    filtrosActivos(valores, porDefectoRef.current, noFiltroRef.current, etiquetas), [valores])

  return { valores, set, setFiltro, setValores, restaurado, avisoFiltro, ocultarAviso,
