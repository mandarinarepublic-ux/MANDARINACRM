...
const TABLES = [
  ...
  },
  {
    ...
    map: (r) => ({
      ...
      monto_pendiente: Math.max(0, toNum(r.MONTO_PENDIENTE) ?? 0), factura_solicitada: toBool(r.FACTURA_SOLICITADA),
...
// Inserta/upserta un bloque; si falla, lo divide a la mitad recursivamente para
// aislar la(s) fila(s) culpable(s) en ~log(n) viajes (evita el fila-por-fila lento
// que, con la latencia iad1↔sa-east-1, agotaba el timeout de 60s).
async function insertChunk(sb, def, rows) {
  if (rows.length === 0) return { ok: 0, failed: [] };
  const q = def.replace
    ? sb.from(def.name).insert(rows)
    : sb.from(def.name).upsert(rows, { onConflict: def.conflict });
  const { error } = await q;
  if (!error) return { ok: rows.length, failed: [] };
  if (rows.length === 1) {
    return { ok: 0, failed: [{ pk: rows[0][def.conflict?.split(',')[0]] ?? '?', error: error.message }] };
  }
  const mid = Math.floor(rows.length / 2);
  const a = await insertChunk(sb, def, rows.slice(0, mid));
  const b = await insertChunk(sb, def, rows.slice(mid));
...
async function writeTable(sb, def, rows) {
  ...
  for (const c of chunk(rows, 500)) {
    const r = await insertChunk(sb, def, c);
    ok += r.ok;
