 ...
 * Detecta la fila de encabezado por `headerKey` (tolera header en fila 0/1/2 +
 * fila-descripción) y filtra filas con la celda `pkCol` vacía.
 */
async function readSheetRobust(sheets, name, headerKey, pkCol) {
...
const TABLES = [
  {
    name: 'usuarios', sheet: 'USUARIOS', headerKey: 'USUARIO_ID', pkCol: 'USUARIO_ID', conflict: 'usuario_id',
    map: async (r) => ({
      ...
      activo: toBool(r.ACTIVO), fecha: toTimestamp(r.FECHA),
    }),
  },
  {
    name: 'clientes', sheet: 'CLIENTES', headerKey: 'CLIENTE_ID', pkCol: 'CLIENTE_ID', conflict: 'cliente_id',
    map: (r) => ({
  ...
  {
    name: 'pedidos', sheet: 'PEDIDOS', headerKey: 'PEDIDO_ID', pkCol: 'PEDIDO_ID', conflict: 'pedido_id',
    map: (r) => ({
      ...
      dias_calculado: toNum(r.DIAS_CALCULADO), dias_prometido: toNum(r.DIAS_PROMETIDO),
  ...
  {
    name: 'detalle_pedido', sheet: 'DETALLE_PEDIDO', headerKey: 'ITEM_ID', pkCol: 'ITEM_ID', conflict: 'item_id',
    map: (r) => ({
      ...
      archivo_diseno: r.ARCHIVO_DISENO || null, shopify_variant_id: r.SHOPIFY_VARIANT_ID || null,
      fecha_modificacion: toTimestamp(r.FECHA_MODIFICACION), notas_area: r.NOTAS_AREA || null,
      eliminado: r.SUBESTADO === 'ELIMINADO',
    }),
  },
  {
    name: 'pagos', sheet: 'PAGOS', headerKey: 'PAGO_ID', pkCol: 'PAGO_ID', conflict: 'pago_id',
  ...
  {
    name: 'guias_despacho', sheet: 'GUIAS_DESPACHO', headerKey: 'GUIA_ID', pkCol: 'GUIA_ID', conflict: 'guia_id',
    map: (r) => ({
  ...
  {
    name: 'dias_entrega', sheet: 'DIAS_ENTREGA', headerKey: 'AREA_COMBINACION', pkCol: 'AREA_COMBINACION', conflict: 'area_combinacion',
...
export async function runBackfill({ only = null, dryRun = false } = {}) {
  ...
  for (const def of targets) {
    let raw;
    try {
      raw = await readSheetRobust(sheets, def.sheet, def.headerKey, def.pkCol);
    } catch (e) {
