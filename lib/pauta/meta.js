...
export async function traerDetalleAnuncios(adAccountId, adIds) {
  ...
  url.searchParams.set('access_token', token())
  // 25 y no 500: al pedir el creativo expandido, Meta responde "Please reduce
  // the amount of data you're asking for" con lotes grandes (probado el 2-ago
  // con 500). El bucle de paginación de abajo ya traía todas las páginas, así
  // que bajar el tamaño no pierde ningún anuncio — solo hace más llamadas.
  url.searchParams.set('limit', '25')
  // El ARTE, no solo el id del creativo.
  //
  // Antes solo se pedía `creative{id,thumbnail_url}` y el cron ni siquiera
  // guardaba la miniatura: las columnas arte_* quedaron NULL en las 374 filas.
  // Sin esto, saber que una venta vino del anuncio "120249665639710600" no le
  // dice nada a nadie — el punto era poder VER qué imagen la produjo.
  //
  // El texto vive en dos sitios según cómo se armó el anuncio: los campos
  // sueltos `body`/`title` (formato viejo) o dentro de `object_story_spec`
  // (link_data para imagen, video_data para video). Se piden los dos y gana el
  // que tenga algo.
  url.searchParams.set('fields', [
    'id', 'name', 'effective_status',
    'creative{id,thumbnail_url,image_url,video_id,body,title,' +
      'object_story_spec{link_data{message,name,picture},video_data{message,title,image_url}}}',
  ].join(','))
  ...
  while (siguiente) {
    ...
    for (const a of j.data || []) {
      ...
      // Para video se usa la miniatura como imagen: el video real necesitaría
      // otra llamada por `video_id` y un permiso extra. Una miniatura que se ve
      // vale más que un reproductor que no carga.
      const esVideo = Boolean(cr.video_id || video.video_id)
      const imagen = cr.image_url || video.image_url || link.picture || cr.thumbnail_url || ''

      porId.set(a.id, {
        estado: a.effective_status || '',
        creativeId: cr.id || '',
        arteUrl: imagen,
        arteTipo: esVideo ? 'video' : (imagen ? 'imagen' : ''),
        arteTexto: cr.body || link.message || video.message || '',
        arteTitular: cr.title || link.name || video.title || '',
      })
    }
    siguiente = j.paging?.next || null
  }
