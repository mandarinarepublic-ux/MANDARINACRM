...
function agrupar(artes) {
  ...
  // El embudo COMPLETO en cada nivel, no solo unos pocos números.
  //
  // Antes se sumaban cinco campos y la pantalla podía mostrar el embudo entero
  // solo en el total de la tienda. Al poder abrir una campaña y preguntarle "¿de
  // dónde salen estas impresiones?", cada nivel tiene que saber responder con
  // los mismos indicadores.
  const sumar = (items) => ({
    // gasto null = "no se sabe", nunca 0. Solo es null si NINGUNO tiene dato:
    // si uno solo lo tiene, el total es al menos eso.
    gasto: items.some((i) => i.gasto != null)
      ? items.reduce((s, i) => s + (i.gasto || 0), 0) : null,
    impresiones: items.reduce((s, i) => s + (i.impresiones || 0), 0),
    clics: items.reduce((s, i) => s + (i.clics || 0), 0),
    ...
    pagados: items.reduce((s, i) => s + (i.pagados || 0), 0),
