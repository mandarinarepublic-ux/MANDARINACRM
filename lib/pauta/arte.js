...
export async function archivarArtePendiente() {
  ...
  try {
    ...
    // Instrumentación: qué anuncios cree que le faltan. Los numeros del resumen
    // llevan horas sin coincidir con lo que dice la base, y sin ver los ids no
    // hay forma de saber si el problema es la seleccion o la escritura.
    res.tanda = tanda.map(([id, u]) => `${id}:${String(u).slice(8, 26)}`)
    ...
    for (const [adId, urlMeta] of tanda) {
      try {
        ...
        // COMPROBAR, no confiar.
        //
        // Hay un caso que no logré explicar: el update reporta filas tocadas y
        // sin error, y sin embargo el anuncio sigue apareciendo como pendiente
        // en la corrida siguiente. Se diagnosticó mal seis veces persiguiendo la
        // causa; lo que no se puede permitir es que siga contándose como éxito.
        //
        // Se relee. Si no quedó marcado, es un error de verdad y sale a la vista
        // en /dashboard/errores, donde se puede subir el arte a mano.
        const { data: verif } = await sb
          .from('pauta_dia').select('ad_id')
          .eq('ad_id', adId).not('arte_archivada_at', 'is', null).limit(1)
        if (!verif?.length) {
          throw new Error('se subió a Cloudinary pero la marca no quedó guardada')
        }
