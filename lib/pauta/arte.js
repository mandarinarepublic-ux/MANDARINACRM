...
export async function archivarArtePendiente() {
  ...
  try {
    ...
    //
    // Los ya archivados se descartan con arte_archivada_at, NO mirando si la URL
    // contiene 'res.cloudinary.com'.
    //
    // Ese fue el bicho: supabase-js traduce el comodín % a * en .ilike() pero NO
    // en .not(col,'ilike',valor). El patrón llegaba literal a PostgREST, no
    // casaba con nada, y el "excluye los archivados" no excluía a nadie: la tanda
    // salía llena de anuncios ya subidos, el archivador reportaba "archivados:
    // 10" y no avanzaba jamás. Sin un solo error, porque volver a subir la misma
    // imagen es idempotente y siempre "funcionaba".
    //
    // Una columna no tiene comodines que interpretar mal.
    const { data, error } = await sb
      .from('pauta_dia')
      .select('ad_id, arte_url')
      .not('arte_url', 'is', null)
      .is('arte_archivada_at', null)
      .order('ad_id')
