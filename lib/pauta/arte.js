...
export async function archivarArtePendiente() {
  ...
  try {
    ...
    //
    // Los ya archivados se descartan EN LA BASE, no acá. Antes se traían todas
    // las filas con arte y se filtraban en JavaScript, y el archivador terminaba
    // reprocesando siempre los mismos 10: PostgREST corta el resultado (max-rows)
    // y el recorte se llevaba justo los pendientes, así que la tanda salía llena
    // de anuncios que ya estaban en Cloudinary. Reportaba "archivados: 10" y no
    // avanzaba nunca — y sin un solo error, porque re-subir es idempotente.
    //
    // Filtrando en la consulta, lo que llega ya son solo pendientes: el recorte
    // deja de importar.
    const { data, error } = await sb
      .from('pauta_dia')
      .select('ad_id, arte_url')
      .not('arte_url', 'is', null)
      .not('arte_url', 'ilike', '%res.cloudinary.com%')
      .order('ad_id')
    if (error) throw error

    const porAd = new Map()
    for (const f of data || []) {
      if (esNuestra(f.arte_url)) continue          // cinturón y tirantes
