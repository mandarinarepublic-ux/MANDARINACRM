...
import { vendedoresDeTienda } from '../canalVenta.js'
...
export async function ventasPorOrigen({ tienda, desde, hasta }) {
  const { data, error } = await getSupabase().rpc('pauta_origen_ventas', {
    ...
    p_vendedores_tienda: [...(await vendedoresDeTienda())],
  })
  if (error) throw new Error(`pauta_origen_ventas: ${error.message}`)

  const out = {
    digital_a_fisico: null, por_chat: null, cliente_de_paso: null,
    mensaje_directo: null, sin_rastro: null,
  }
...
/**
 * Los pedidos de UN origen, para poder abrirlos y rastrearlos uno por uno.
 *
 * Devuelve el celular a propósito: con eso se busca la conversación en el inbox
 * y se ve de dónde salió el cliente. Es justo lo que hay que hacer con los
 * `mensaje_directo` — tienen chat, solo que no empezó en un anuncio.
 */
export async function pedidosPorOrigen({ tienda, desde, hasta, origen }) {
  const { data, error } = await getSupabase().rpc('pauta_pedidos_por_origen', {
    p_desde: desde, p_hasta: hasta, p_tienda: tienda, p_origen: origen,
    p_vendedores_tienda: [...(await vendedoresDeTienda())],
  })
  if (error) throw new Error(`pauta_pedidos_por_origen: ${error.message}`)
  return (data || []).map((p) => ({
    pedidoId: p.pedido_id, fecha: p.fecha, cliente: p.cliente, celular: p.celular,
    vendedor: p.vendedor, monto: Number(p.monto), estadoPago: p.estado_pago,
    tuvoChat: p.tuvo_chat,
  }))
}

/**
 * Los pedidos que produjo UN anuncio. El último eslabón del rastreo: gasto →
 * creativo → chat → pedido con nombre y apellido.
 *
 * Usa el mismo criterio que pauta_embudo (mismo VENTANA_DIAS), o el detalle no
 * cuadraría con el total que muestra la fila del anuncio.
 */
export async function pedidosDeAnuncio({ tienda, desde, hasta, adId }) {
  const { data, error } = await getSupabase().rpc('pauta_pedidos_de_anuncio', {
    p_desde: desde, p_hasta: hasta, p_tienda: tienda,
    p_ad_id: adId, p_ventana_dias: VENTANA_DIAS,
  })
  if (error) throw new Error(`pauta_pedidos_de_anuncio: ${error.message}`)
  return (data || []).map((p) => ({
    pedidoId: p.pedido_id, fecha: p.fecha, cliente: p.cliente, celular: p.celular,
    vendedor: p.vendedor, monto: Number(p.monto), estadoPago: p.estado_pago,
    diasHastaCompra: p.dias_hasta_compra == null ? null : Number(p.dias_hasta_compra),
  }))
}
