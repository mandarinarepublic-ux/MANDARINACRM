 ...
 *
 * @param {{item:object, areaRol:?string, estadoNuevo:string, usuario:?string,
 *          trabado?:boolean}} args  `trabado`: el pedido tiene traba de corte
 *          puesta a mano, así que no se registra corte automático.
 * @returns {object[]} vacío si no hay nada que registrar
 */
export function eventosDelCambio({ item, areaRol, estadoNuevo, usuario, trabado = false }) {
  ...
  // El mismo `trabado` que corta el auto-marcado en `debeAutoMarcarCorte`: si
  // acá se registrara el evento igual, la tabla diría CORTADO mientras la prenda
  // sigue pendiente. Dos verdades distintas sobre la misma prenda.
  const yaCortada = String(item.SUBESTADO_CORTE ?? '').trim().toUpperCase() === 'CORTADO'
  if (!trabado && !yaCortada && implicaCorte(despues)) {
 ...
 *
 * ☠️ `trabado` es el pedido con traba de corte puesta a mano («falta algo», ver
 * lib/cortePedido.js). Mientras esté trabado, el auto-marcado NO corre: si
 * corriera, el pedido que el cortador acaba de trabar se le iría de la bandeja
 * en cuanto el área tocara la prenda — que es exactamente el defecto que la
 * traba viene a cerrar. Y el contador diría «6/6 cortados» sin que nadie haya
 * cortado nada.
 *
 * @param {object} item
 * @param {string} estadoNuevo
 * @param {{trabado?:boolean}} [opciones]
 */
export function debeAutoMarcarCorte(item, estadoNuevo, { trabado = false } = {}) {
