...
export function encajeEnA4(anchoPx, altoPx, tolerancia = TOLERANCIA_UNA_HOJA) {
  ...
  // Con la sobra despreciable: el documento se dibuja con la proporción exacta
  // de un A4 (CotizacionPreview, aspect-ratio 210/297) y el redondeo de
  // subpíxel lo deja en 297.02. Eso es una hoja, no una hoja "encogida" al
  // 99.99% con un margen de 0.01 mm a cada lado.
  if (altoMm <= A4_MM.alto * (1 + SOBRA_DESPRECIABLE)) {
    return { modo: 'una-hoja', x: 0, y: 0, ancho: A4_MM.ancho, alto: Math.min(altoMm, A4_MM.alto), hojas: 1, escala: 1 }
  }
