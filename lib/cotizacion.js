 ...
 */
/**
 * El `entrega_dias` de la raíz, con 15 como default cuando NO hay valor.
 *
 * ☠️ Hallazgo 4: `Number(c?.entrega_dias) || 15` convertía un 0 legítimo en
 * 15 (el formulario permite `min={0}`). El arreglo directo, `Number.isFinite`,
 * cambió un bug por otro (regresión de la ronda 2 de revisión): `Number(null)`
 * es `0` y `Number.isFinite(0)` es `true`, así que un `entrega_dias` en NULL
 * —una fila real de la base— pasaba a valer 0 en vez de 15. El viejo `|| 15`
 * atrapaba `null`/`''`/`false` por accidente, junto con el 0 que no debía.
 *
 * Acá se distingue «no hay valor» (`null`, `undefined`, `''`, `false`) de
 * «el valor es 0» ANTES de convertir a número: solo lo primero cae a 15.
 */
function entregaDiasRaiz(c) {
  const v = c?.entrega_dias
  if (v === null || v === undefined || v === '' || v === false) return 15
  const n = Number(v)
...
export function opcionesDe(c) {
  ...
  return [{
    id: ID_OPCION_UNICA,
    nombre: '',
    entrega_dias: entregaDiasRaiz(c),
