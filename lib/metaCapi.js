 ...
 */
export async function enviarPurchase({ pedidoId, tiendaId, cliente, montoTotal, eventTime }) {
  ...
  const evento = {
    ...
    // 'chat' es el comportamiento de siempre y se mantiene para las ventas que no
    // vienen de pauta (la mayoría): cambiarlo ahí no aportaría nada y rompería el
    // histórico de señales que Meta ya tiene.
    action_source: atribuido ? 'business_messaging' : 'chat',
    ...(atribuido && { messaging_channel: 'whatsapp' }),
    user_data: atribuido
  ...
  try {
    ...
    if (!res.ok) {
      ...
      registrarEvento({ fuente: 'meta', nivel: 'error', mensaje: detalle, pedidoId, detalle: { pixelId, atribuido } })
    ...
    registrarEvento({
      ...
      mensaje: `Purchase recibido (${recibidos})${atribuido ? ' · atribuido a pauta' : ''}`,
      detalle: { pixelId, atribuido, wabaId: wabaId || undefined },
    })
    return { ok: true, recibidos, pixelId, atribuido }
  } catch (e) {
