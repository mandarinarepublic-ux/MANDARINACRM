...
export async function enviarPurchase({ pedidoId, tiendaId, cliente, montoTotal, eventTime }) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      registrarEvento({ fuente: 'meta', nivel: 'error', mensaje: detalle, pedidoId, detalle: { pixelId } })
      return { ok: false, error: detalle }
    }
    const recibidos = body?.events_received ?? 0
    registrarEvento({ fuente: 'meta', nivel: 'ok', mensaje: `Purchase recibido (${recibidos})`, pedidoId, detalle: { pixelId } })
    return { ok: true, recibidos, pixelId }
  } catch (e) {
