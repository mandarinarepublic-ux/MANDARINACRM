...
const PAGOS   = ['TODOS','PENDIENTE','ABONO','PAGADO']
const PAGO_LABELS = { TODOS: 'Todos los pagos', PENDIENTE: '⚠️ Pago pendiente', ABONO: '🔶 Con abono', PAGADO: '✅ Pagado' }
const PAGO_STYLES = {
  TODOS:    'border-gray-700 text-gray-500',
  PENDIENTE:'border-red-500    text-red-400    bg-red-500/10',
  ABONO:    'border-yellow-500 text-yellow-400 bg-yellow-500/10',
  PAGADO:   'border-green-500  text-green-400  bg-green-500/10',
}
const PAGE_SIZE = 30
...
export default function HistorialPage() {
  ...
  useEffect(() => {
    ...
    if (u.rol !== 'VENDEDOR_YAW') {
      try {
        ...
        if (f.filtroPago)   setFiltroPago(f.filtroPago)
  ...
  useEffect(() => {
    if (!user) return
    localStorage.setItem(LS_FILTROS, JSON.stringify({ filtroEstado, filtroTienda, filtroPago, fechaDesde, fechaHasta }))
  }, [user, filtroEstado, filtroTienda, filtroPago, fechaDesde, fechaHasta])
  ...
  // Contadores por estado de pago (sobre lista filtrada sin filtroPago)
  const pagoContadores = pedidos.reduce((acc, p) => {
    const estadoBase = (() => {
      if (filtroEstado !== 'TODOS' && p.ESTADO_PEDIDO !== filtroEstado) return null
      if (!isYAW && filtroTienda !== 'TODAS' && p.TIENDA_ID !== filtroTienda) return null
      if (busquedaDebounced && !coincideBusqueda(p, busquedaDebounced)) return null
      if (fechaDesde) { const f = parseFecha(p.FECHA_PEDIDO); if (!f || f < new Date(fechaDesde)) return null }
      if (fechaHasta) { const f = parseFecha(p.FECHA_PEDIDO); const h = new Date(fechaHasta); h.setHours(23,59,59); if (!f || f > h) return null }
      return p.ESTADO_PAGO || 'PENDIENTE'
    })()
    if (estadoBase) acc[estadoBase] = (acc[estadoBase] || 0) + 1
    return acc
  }, {})
  ...
  const pagoColorPill = {
    PAGADO:   'text-green-400 bg-green-500/10',
    ABONO:    'text-yellow-400 bg-yellow-500/10',
    PENDIENTE:'text-red-400 bg-red-500/10',
  }
      ...
      <div className="sticky top-0 z-10 bg-gray-950 border-b border-gray-800 px-4 pt-4 pb-3">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex gap-2 mt-2 overflow-x-auto pb-1 scrollbar-none">
            {PAGOS.map(pg => {
              const count = pg === 'TODOS'
                ? Object.values(pagoContadores).reduce((a, b) => a + b, 0)
                : pagoContadores[pg] || 0
              const isActive = filtroPago === pg
              return (
                <button key={pg} onClick={() => setFiltroPago(pg)}
                  className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border transition-all flex-shrink-0 flex items-center gap-1.5
                    ${isActive
                      ? pg === 'PENDIENTE' ? 'bg-red-500    border-red-500    text-white'
                      : pg === 'ABONO'     ? 'bg-yellow-500 border-yellow-500 text-black'
                      : pg === 'PAGADO'    ? 'bg-green-600  border-green-600  text-white'
                                           : 'bg-gray-700   border-gray-700   text-white'
                      : pg === 'PENDIENTE' ? 'border-red-800    text-red-500'
                      : pg === 'ABONO'     ? 'border-yellow-800 text-yellow-500'
                      : pg === 'PAGADO'    ? 'border-green-800  text-green-500'
                                           : 'border-gray-700   text-gray-500'}`}>
                  {pg === 'TODOS'     ? '💳 Cobro' :
                   pg === 'PENDIENTE' ? '⚠️ Pendiente' :
                   pg === 'ABONO'     ? '🔶 Abono' :
                                        '✅ Pagado'}
                  {count > 0 && (
                    ...
                    <div key={p.PEDIDO_ID} className="card overflow-hidden">
                      ...
                      <button
                        ...
                        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                          ...
                          <span className={`text-xs px-2 py-0.5 rounded-full ${pagoColorPill[p.ESTADO_PAGO] || 'text-red-400 bg-red-500/10'}`}>
                            ${parseFloat(p.MONTO_TOTAL||0).toFixed(2)}
                            {p.ESTADO_PAGO === 'PENDIENTE' && ' ⚠️'}
                            {p.ESTADO_PAGO === 'ABONO'     && ' 🔶'}
                          </span>
                        </div>
