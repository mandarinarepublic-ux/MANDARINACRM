  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`text-xs font-bold text-white px-3 py-1 rounded-full ${CORTE_CONFIG[subestadoCorte]?.color}`}>
          {CORTE_CONFIG[subestadoCorte]?.label}
        </span>
      </div>
      ...
      <div className="flex gap-4">
        <div className="w-32 flex-shrink-0">
          {fotos.length > 0 ? (
            <>
              <div className="w-32 h-32 rounded-xl overflow-hidden border border-gray-700 bg-gray-800 mb-2 cursor-pointer"
            ...
            <div className="w-32 h-32 rounded-xl border border-gray-800 bg-gray-800/50 flex items-center justify-center">
        ...
        <div className="flex-1 flex flex-col gap-2">
          {item.DETALLE_PERSONALIZADO && (
            <div className="text-xs bg-gray-800/50 rounded-xl px-3 py-2">
            ...
            {CORTE_ORDEN.map(s => (
              <button key={s} onClick={() => handleCorte(s)}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all
                  ${subestadoCorte === s
                    ? `${CORTE_CONFIG[s].color} text-white shadow-lg`
