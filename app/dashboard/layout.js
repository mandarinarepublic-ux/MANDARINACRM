...
export default function DashboardLayout({ children }) {
  ...
  const tiendaColor = user.tiendas?.includes('INDSTORE') && !user.tiendas?.includes('MANDARINA')
    ? '#1A1A2E' : '#FF6B00'

  return (
    <div className="min-h-screen bg-gray-950 flex">
      {/* Sidebar desktop */}
      <aside className="hidden md:flex flex-col w-60 bg-gray-900 border-r border-gray-800 p-4 fixed h-full">
        {/* Brand */}
        <div className="flex items-center gap-3 mb-8 px-2">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white font-bold text-sm"
               style={{ backgroundColor: tiendaColor }}>M</div>
          <div>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="text-gray-400 p-1">
          {menuOpen ? '✕' : '☰'}
        </button>
      </div>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-gray-950/95 pt-14 p-4">
